...
import { requireNativeComponent, ViewProps, ViewPropTypes,Platform,UIManager,findNodeHandle } from "react-native";
import {
  LatLngPropType,
  LocationPropType,
  mapEventsPropType
...
export default class MapView extends Component<Props> {
  ...
  removeAllOverlay() {
    this.callNativeMethod('removeAllOverlay',null);
  }
  ...
  callNativeMethod = (command, params) => {
    if (Platform.OS == 'ios') {
      UIManager.dispatchViewManagerCommand(
        findNodeHandle(this),
        UIManager.getViewManagerConfig("BaiduMapView").Commands[command],
        params ? [params] : null,
      );
    }
  };
